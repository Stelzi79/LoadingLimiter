	...
	if compare_tables983(copiedoutput, output) then
			...
			if global.signals983[signal983] then
				if type (value983) == "number" then
					if value983 >= -2147483648 and value983 <= 2147483647 then
						actual_output983[i983]={signal={type=global.signals983[signal983], name=signal983},count=(tonumber(value983) or 0),index=i983}
						i983=i983+1
					else
						tbl983.errors2 =tbl983.errors2.."  +++output value must be between -2147483648 and 2147483647 ("..signal983..")"
		...
		if tbl983.errors..tbl983.errors2 ~="" then
			for _, player in pairs(game.players) do
				player.add_custom_alert(tbl983.entity,{type="virtual", name="luacom_error"},"LuaCombinator Error: "..tbl983.errors..tbl983.errors2,true)
			end
		end
...
script.on_event(defines.events.on_pre_ghost_deconstructed, function(event)
	if event.ghost and event.ghost.type == "entity-ghost" and event.ghost.ghost_name == "lua-combinator3" then
		local spot= event.ghost.surface.find_entities_filtered{name="entity-ghost", inner_name="luacom_blueprint_data", ghost_name="luacom_blueprint_data", area= {{event.ghost.position.x-0.1,event.ghost.position.y-0.1},{event.ghost.position.x+0.1,event.ghost.position.y+0.1}}}
		if #spot >0 then
			spot[1].destroy()
		end
	end
...
script.on_event(defines.events.on_player_mined_entity, function(event)
	if event.entity  and event.entity.name == "entity-ghost" and event.entity.ghost_name == "lua-combinator3" then
		local spot= event.entity .surface.find_entities_filtered{name="entity-ghost", inner_name="luacom_blueprint_data", ghost_name="luacom_blueprint_data", area= {{event.entity.position.x-0.1,event.entity.position.y-0.1},{event.entity.position.x+0.1,event.entity.position.y+0.1}}}
		if #spot >0 then
			spot[1].destroy()
		end
	end
	...
	if (settings.global["luacom-colorize-code"] and settings.global["luacom-colorize-code"].value) then
		...
		while i<=#res do
			if string.sub(res,i,i) == "[" and (string.sub(res,i,i+5) =="[font=" or string.sub(res,i,i+6) == "[color=") then
				...
				colored=colored + 1
			elseif string.sub(res,i,i) == "[" and (--[[string.sub(res,i,i+5) =="[/font" or --]]string.sub(res,i,i+6) == "[/color") then
				if colored == 1 then
					result = result..string.sub(res,i,i)
				else
					for j=1,50 do
...
function find_difference983 (current, cache)
	...
	for i=1, math.max(#current, #cache) do
		...
		if current:sub(i,i) ~= cache:sub(i,i) then
			...
			if lastletter == current:sub(i,i) or (#current >i and current:sub(i+1,i+1) == current:sub(i,i) and not (#current <#cache and cache:sub(i,i)~=cache:sub(i+1,i+1))) or (#current <#cache and lastletter == cache:sub(i,i))then --multiple letters after another
...
function find_difference_multi983 (current, cache)
	...
	while true do
		...
		if current:sub(currentchar,currentchar) ~= cache:sub(cachechar,cachechar) then
			...
			--print983("lastletter: "..lastletter)
			--print983("currentletter: "..current:sub(i,i))
			if lastletter == current:sub(currentchar,currentchar) or (#current >currentchar and current:sub(currentchar+1,currentchar+1) == current:sub(currentchar,currentchar) and not (#current <#cache and cache:sub(cachechar,cachechar)~=cache:sub(cachechar+1,cachechar+1))) or (#current <#cache and lastletter == cache:sub(cachechar,cachechar))then --multiple letters after another
				--if lastletter == current:sub(i,i) then
				--	print983("(lastletter == current:sub(i,i))")
				--elseif (#current >i and current:sub(i+1,i+1) == current:sub(i,i)) then
				--	print983("1\""..current:sub(i,i).."\"")
...
script.on_event(defines.events.on_gui_text_changed, function(event) 
	...
	if event.element.name == "luacom3_code" then
		...
		if not event.element.parent.parent.parent.parent.parent.flow.luacom3_formatting.state or(not settings.global["luacom-indent-code"].value and not (settings.global["luacom-colorize-code"] and settings.global["luacom-colorize-code"].value)) then
		...
		if not global.textboxes983[event.element.parent.parent.parent.parent.parent.name] then
			...
			event.element.style.minimal_height = math.max(120,math.min(game.players[event.player_index].display_resolution.height*0.4,returns983*20+40))
			...
			if cursor then
				...
				if current~= cache and (not noupdate and not (math.abs(#current+spacefinder-#cache) >1) or updateanyway)  then
					...
					if (settings.global["luacom-colorize-code"] and settings.global["luacom-colorize-code"].value) then
						...
						current = remove_formatting983(current:sub(1,cursor))--.."a"):sub(1,-2)
						...
						while i<=#formatted do
							if string.sub(formatted,i,i) == "[" and (string.sub(formatted,i,i+5) =="[font=" or string.sub(formatted,i,i+6) == "[color=" or string.sub(formatted,i,i+5) =="[/font" or string.sub(formatted,i,i+6) == "[/color") then
								--print983("tag found"..temp)
								--temp=i
								for j=1,25 do
...
on_built_entity983 = function (event)
	if event.created_entity.valid and event.created_entity.name == "lua-combinator3" then
		global.counter983 = global.counter983 + 1
		global.combinators983[event.created_entity.unit_number] = {["formatting"] = true, ["entity"] = event.created_entity, ["code"]="", ["func"] = nil, ["variables"] = {}, ["errors"]="", ["errors2"]="", ["output"]={}, ["next_tick"]=1, ["last_user"] = event.created_entity.last_user.index, number = global.counter983}
		if string.sub(game.active_mods["base"],1,4) == "0.16" then
			global.combinators983[event.created_entity.unit_number].code = "--hi, maybe don't remove this line because it disappears when you hit the line limit :)\n"
		end
		local blueprint_data983 = event.created_entity.surface.find_entities_filtered{position = event.created_entity.position, ghost_name = "luacom_blueprint_data"}
		if blueprint_data983[1] then
			...
			global.combinators983[event.created_entity.unit_number].blueprint_data = event.created_entity.surface.create_entity{name = "luacom_blueprint_data", position = event.created_entity.position, force = event.created_entity.force}
			...
			load_code983(global.combinators983[event.created_entity.unit_number]["code"], event.created_entity.unit_number,event)
		else
			global.combinators983[event.created_entity.unit_number].blueprint_data = event.created_entity.surface.create_entity{name = "luacom_blueprint_data", position = event.created_entity.position, force = event.created_entity.force}
			global.combinators983[event.created_entity.unit_number].blueprint_data.destructible = false
			global.combinators983[event.created_entity.unit_number].blueprint_data.minable = false
		end
		write_to_combinator(global.combinators983[event.created_entity.unit_number].blueprint_data,global.combinators983[event.created_entity.unit_number]["code"])
	end
	combinator_event(event)
end
...
script.on_event(defines.events.on_tick, function (event)
	for unit_nr983, gui983 in pairs(global.guis983) do
		if (not global.combinators983[unit_nr983]) or (not global.combinators983[unit_nr983].entity.valid) or (not gui983.valid) then
			gui983.destroy()
			global.guis983[unit_nr983]=nil
		else
			...
			gui983.main_table.left_table.under_text.errors.caption=(global.combinators983[unit_nr983]["errors"] or "")..(global.combinators983[unit_nr983]["errors2"] or "")
		end
	end
...
script.on_event(defines.events.on_gui_opened, function(event)
	...
	if player.opened ~= nil and player.opened.name == "lua-combinator3" then
...
function create_gui(player, entity)
	local gui = player.gui.center.add{type = "frame", name = "luacom3_gui_"..entity.unit_number, caption = "", direction = "vertical"}
		gui.caption = "["..global.combinators983[entity.unit_number].number.."] combinator, rednet[], greennet[], var[], output[], delay"
	...
	elem = gui.main_table.left_table.code_scroll.add{type = "table", column_count=1, name = "code_table", direction = "vertical"}
		--elem.style.vertically_stretchable = true
		--elem.style.vertically_squashable = false
	elem = gui.main_table.left_table.code_scroll.code_table.add{type = "text-box", name = "luacom3_code", direction = "vertical"}
		elem.style.vertically_stretchable = true
		--elem.style.vertically_squashable = false
	...
	elem =gui.main_table.left_table.under_text.add{type = "button", name = "luacom3_ok_"..entity.unit_number, direction = "horizontal",caption="ok"}
		elem.style.width=35
...
function helper_gui(pl)
	...
	if not player.gui.center["luacom3_helper_"..pl] then
		local gui = player.gui.center.add{type = "frame", name = "luacom3_helper_"..pl, caption = "", direction = "vertical"}
		...
		gui.add{type = "label", name = "8", direction = "horizontal",caption = "delay = the delay in ticks until the next on_tick update (to save some ups) (not persistent, needs to be set on each update)"}
		...
		gui.add{type = "label", name = "15", direction = "horizontal",caption = "You can use any event except on_init and on_load... For example:"}
		gui.add{type = "label", name = "16", direction = "horizontal",caption = "on_nth_tick[7] = function (event) ... (on_nth_tick has this quite special syntax)"}
		gui.add{type = "label", name = "17", direction = "horizontal",caption = "The combinator basically executes your code once and registers your 'event variables' to the corresponding events"}
	end
end
		...
		if event.button == defines.mouse_button_type.left then
			if global.presets983[id] then
				if event.element.parent.luacom3_formatting.state then
					event.element.parent.parent.main_table.left_table.code_scroll.code_table.luacom3_code.text = format_code983(global.presets983[id])
				else
					event.element.parent.parent.main_table.left_table.code_scroll.code_table.luacom3_code.text = global.presets983[id]
				end
				global.textboxes983[event.element.parent.parent.name] = event.element.parent.parent.main_table.left_table.code_scroll.code_table.luacom3_code.text
				if not global.history983[event.element.parent.parent.name] then 
					global.history983[event.element.parent.parent.name] = {event.element.parent.parent.main_table.left_table.code_scroll.code_table.luacom3_code.text} 
					global.historystate983[event.element.parent.parent.name] = 1
				else
					insert_history983(event.element.parent.parent, event.element.parent.parent.main_table.left_table.code_scroll.code_table.luacom3_code.text)
				end
			else
				global.presets983[id] = remove_colors983(event.element.parent.parent.main_table.left_table.code_scroll.code_table.luacom3_code.text)
				if string.sub(game.active_mods["base"],1,4) == "0.16" then
	...
	elseif event.element.name=="luacom3_formatting" then
		if event.button == defines.mouse_button_type.left then
			if event.element.state == true then
				event.element.parent.parent.main_table.left_table.code_scroll.code_table.luacom3_code.text = format_code983(event.element.parent.parent.main_table.left_table.code_scroll.code_table.luacom3_code.text)
			else
				event.element.parent.parent.main_table.left_table.code_scroll.code_table.luacom3_code.text = remove_colors983(event.element.parent.parent.main_table.left_table.code_scroll.code_table.luacom3_code.text)
			end
			local combinator = tonumber(string.sub(event.element.parent.parent.name,13))
...
script.on_event(defines.events.on_entity_settings_pasted, function(event)
	if event.source.name == "lua-combinator3" and event.destination.name == "lua-combinator3" then
		global.combinators983[event.destination.unit_number].code = global.combinators983[event.source.unit_number].code
		global.combinators983[event.destination.unit_number].variables = luacom_deepcopy(global.combinators983[event.source.unit_number].variables)
		global.combinators983[event.destination.unit_number].errors2 = global.combinators983[event.source.unit_number].errors2
		global.combinators983[event.destination.unit_number].output = luacom_deepcopy(global.combinators983[event.source.unit_number].output)
		global.combinators983[event.destination.unit_number].next_tick = global.combinators983[event.source.unit_number].next_tick
		if not 	   global.textboxes983["luacom3_gui_"..event.destination.unit_number] then
			global.textboxes983["luacom3_gui_"..event.destination.unit_number] = global.combinators983[event.source.unit_number].code
		end
		if not global.history983["luacom3_gui_"..event.destination.unit_number] then 
			global.history983["luacom3_gui_"..event.destination.unit_number] = {global.combinators983[event.source.unit_number].code} 
			global.historystate983["luacom3_gui_"..event.destination.unit_number] = 1
		else
			insert_history983 ("luacom3_gui_"..event.destination.unit_number,global.combinators983[event.source.unit_number].code)
		end
		...
		global.combinators983[event.destination.unit_number].formatting = global.combinators983[event.source.unit_number].formatting
...
function load_code983(code,id983,e983)
	...
	global.combinators983[id983]["func"],global.combinators983[id983]["errors"] = load(global.combinators983[id983]["code"])
	...
	if global.combinators983[id983]["func"] then
		...
		local _,error983 = combinator_process(e983,id983,global.combinators983[id983]["func"])--pcall(global.combinators983[id983]["func"],event)
		if error983 then
